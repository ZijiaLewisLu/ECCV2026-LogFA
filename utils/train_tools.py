...
from .evaluate import Checkpoint, Video
...
def compute_null_weight(cfg, dataset, verbose=True):
    """
    normalized the frequency of null class to 1/num_classes
    """
    average_trans_len = dataset.average_transcript_len
    if average_trans_len is None:
        raise ValueError("Dataset must provide average_transcript_len")
    if verbose:
        print("Average transcript length", average_trans_len)
...
def save_results(
    ...
    videos = []
    for index, vname in enumerate(vnames):
        video = Video(
            vname,
            gt_label=to_numpy(label_list[index]),
            **attrs_saves[index],
        )
        videos.append(video)
